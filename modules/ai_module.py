...
def fallback_suggestions(user_data):
    ...
    if user_data["transport_co2"] > 5:
        tips.append(
            "Use public transport or reduce unnecessary travel"
        )

    if user_data["electricity_co2"] > 5:
        tips.append(
            "Reduce electricity usage by switching off devices"
        )

    if user_data["waste_co2"] > 2:
        tips.append(
            "Recycle more and reduce waste generation"
        )
